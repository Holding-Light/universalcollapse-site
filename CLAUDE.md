...
**Read pages are generated.** pandoc output. Safe to rebuild.

**Landing pages contain authored prose that exists nowhere else.**
`landing_kernelfirst.html` is 14K carrying bespoke editorial sections —
"Kernel-first, not primitive-first", "The Observer's Fallacy", "What would make it
wrong". None of it appears in the read page (grep: 2 hits landing, 0 read). It is
**not** derivable from the docx.

**Do not "regenerate" a landing page. You will destroy authored work.**

Open question, first task worth doing: memory records a `build_data.py` +
`generate_pages.py` pipeline for landings. Determine whether that pipeline sources
the per-paper prose from a data file (Rule 3 is closer than feared — the prose is
already data) or whether landings are hand-maintained (Rule 3 does not hold for
landings and §7.2 needs an operator decision). **Check before assuming either.**

Evidence that they drift: `citation_pdf_url` is present on **2 of 17** landings
(kernel_first, wp01). That is the signature of hand-maintenance.
